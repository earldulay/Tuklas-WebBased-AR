...
export function toPersistedModule(module: (typeof modules)[number]) {
  const { predictions, overview: _overview, ...persisted } = module;
  return {
    ...persisted,
    prediction: predictions[0].question,
    choices: predictions[0].choices,
  };
}
